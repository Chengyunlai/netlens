...
> 上面的输出取自一次真实探测，用来说明目标形态。正式实现会重做渲染层，但"分层 + 冷热对比 + 诊断"这个结构不会变。
...
**让每一个 Web 开发者，都能看懂自己站点背后的网络层。**

今天的性能工具很擅长给出结论——LCP 3.2 秒、性能评分 83——但不擅长解释结论的来源。当一个页面变慢，开发者手里是一串指标，而不是一条因果链：是 DNS 慢，还是 RTT 高？是连接没有复用，还是跨了太多 Origin？还是服务端本身就在拖后腿？

浏览器和操作系统其实已经把这些事实告诉我们了，只是它们散落在 socket 事件、TLS 握手信息和响应头里，需要有人拼起来。`netlens` 想做的就是把它们拼成一句人能直接读懂的话，并指向一个具体动作。

它还有第二个野心：**让诊断过程本身就是一次学习**。每一条结论都带一个理论锚点——为什么 TCP 建连大约要花掉一个 RTT、为什么 TLS 1.3 比 1.2 少一个往返、为什么连接复用能省掉整段建连成本。用工具的人，顺手就把这些概念过了一遍。
